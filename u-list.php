    ...
    <div class="container">
      <div class="row">
        <div class="row s12">
          <h3>会計ユーザ一覧</h3>
          <table>
            <tr><th>ユーザID</th><th>氏名</th></tr>
            <?php
                require_once('config/config.php');
                $sql = mysqli_query($db_link, "SELECT UserID, UserName FROM tp_user_booth");
                while($result = mysqli_fetch_assoc($sql)) {
                    print("<tr><td>".$result['UserID']."</td><td>".$result['UserName']."</td></tr>");
                }
            ?>
          </table>
          <h3>団体ユーザ一覧</h3>
          <?php
            ...
            while($result = mysqli_fetch_assoc($sql)) {
                ...
                $orgid = $result['OrgID'];
                $sql2 = mysqli_query($db_link, "SELECT UserID, UserName FROM tp_user_org WHERE OrgID = '$orgid'");
                print("<table><tr><th>ユーザID</th><th>氏名</th></tr>");
                while($result2 = mysqli_fetch_assoc($sql2)) {
                    print("<tr><td>".$result2['UserID']."</td><td>".$result2['UserName']."</td></tr>");
                }
                print('</table>');
            }
          ?>
          <h3>顧客ユーザ一覧</h3>
          <table>
            <tr><th>ユーザID</th><th>ニックネーム</th></tr>
            <?php
                $sql = mysqli_query($db_link, "SELECT UserID, UserName FROM tp_user_cust");
                while($result = mysqli_fetch_assoc($sql)) {
                    print("<tr><td>".$result['UserID']."</td><td>".$result['UserName']."</td></tr>");
                }
            ?>
