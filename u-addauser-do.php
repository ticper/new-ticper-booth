  ...
  if(isset($_SESSION['UserID']) == '') {
    print("<script>location.href = 'index.php';</script>");
        ...
        if ($h_userid == '' or $h_username == '' or $h_password == '') {
            print('<script>alert("入力されていない項目があります。"); location.href = "u-addauser.php";</script>');
        } else { //全て入力されている場合
            // データベースにユーザを登録
            $sql = mysqli_query($db_link, "INSERT INTO tp_user_all VALUES ('$h_userid', '$h_username', '$hashed_password')");
            // データベースへの登録に失敗した場合
            if (!$sql) {
                print('<script>alert("データベースへの登録に失敗しました。"); location.href = "u-addauser.php";</script>');
            } else { // 成功した場合
                print('<script>alert("登録しました。"); location.href = "u-addauser.php";</script>');
            }
        }
    }
?>